...
The eight works above comprise four 16:9 landscape left–right samples and four 3:4 portrait top–bottom samples. Every work was generated independently by Panel 082 from its own original brief; no artwork from another numbered Panel is reused. The samples use intelligent English copy.

<!-- xxd-human-intro:start -->
## What it solves

Flat children's illustration can become sticker-like, while realistic 3D loses the breathing quality of watercolour. If a complex scene is not reduced, its central character and visual metaphor disappear.

**Panel 082** compresses the photograph into broad irregular watercolour fields, then lets one protagonist rise from them through Naïve + Wonky contours and clear Isometric / 2.5D structure. Layered planes, occlusion, and small height shifts create space; source-derived vivid colour and broad warm-white paper preserve air and playfulness.

### Best for

- making one subject leap from an irregular watercolour field through Isometric / 2.5D depth;
- keeping childlike Naïve + Wonky character without losing structural direction, layering, or spatial logic;
- reducing complex photographs to identity, metaphor, and only the most life-filled source colours;
- top-bottom, left-right, design-only, multi-ratio, wallpaper, and directory-batch delivery.

### Quick start

> Use XXD Panel 082 on this image and recommend the most suitable composition and size first.
...
**Signature:** irregular watercolour field · Naïve + Wonky · Isometric / 2.5D · childlike contour · vivid healing colour · tiny spatial world
...
|---|---|
| **What you get** | A premium picture-book poster whose three-dimensional protagonist rises from a soft watercolour plane |
| **Recognisable signature** | Irregular watercolour base, naïve contours, clear axonometric direction, layered planes, and broad negative space |
| **How it respects the source** | It keeps only the strongest identity traits and visual metaphor, and retunes the brightest, most vivid source colours |
...
```text
extract identity, structure, pose, and visual metaphor → compress the scene into irregular watercolour fields → establish one Naïve + Wonky protagonist → build Isometric / 2.5D depth through layered planes, facets, and occlusion → retune the most vivid source colours → preserve broad warm-white space → place sparse rounded editorial text along fields and axes
```

## Recognisable finished traits

- Large irregular watercolour fields show tide marks, grain, transparency, and slight misregistration.
- One recognisable protagonist rises from the flat field through axonometric direction, layered planes, and occlusion.
- Naïve proportions and trembling contours remain controlled by clear Isometric spatial logic.
- The palette comes from the brightest source colours, with a softer ground and slightly more solid key subject colours.
- Complex environments disappear so positive and negative shapes, air, and one visual centre control the page.

All aesthetic constraints live only in the [canonical prompt](references/original-prompt/zh-CN.md). The Skill and runtime adapters resolve delivery variables. [Skill workflow](SKILL.md) · [English runtime adapter](references/xxd-panel-082-prompt.en.md)
...
```text
/xxd-panel-082 photo.jpg --mode top-bottom,design-only --size auto,3:4,9:16 --text prompt --locale ja-JP
```
...
```text
/xxd-panel-082 photo.jpg --prefs last
/xxd-panel-082 photo.jpg --prefs edit
/xxd-panel-082 photo.jpg --prefs new
```
...
/xxd-panel-082 photo.jpg --mode top-bottom --size auto --text prompt --locale en-GB

# Left-right comparison, 3:2 landscape, no text
/xxd-panel-082 photo.jpg --mode left-right --size 3:2 --text none

# Design-only, 9:16 phone portrait, Japanese text
/xxd-panel-082 photo.jpg --mode design-only --size 9:16 --text prompt --locale ja-JP

# Linked wallpaper pack: establish one anchor, then recompose every device
/xxd-panel-082 photo.jpg --mode wallpaper-pack --wallpaper linked --text none

# Independent wallpapers with exact device resolutions
/xxd-panel-082 photo.jpg --mode wallpaper-pack --wallpaper independent --wallpaper-size phone=1440x3200,ipad=2048x2732,desktop=3840x2160,watch=1024x1024 --text none

# Follow the source image's aspect ratio
/xxd-panel-082 photo.jpg --mode design-only --size source --text none

# Create square, portrait, and landscape compositions
/xxd-panel-082 photo.jpg --mode design-only --size 1:1,3:4,16:9 --text none

# Combine a custom ratio with exact pixel targets
/xxd-panel-082 photo.jpg --mode design-only --size 11:14,2160x3840,3840x2160 --text none

# Let the original prompt generate wording; constrain only the locale
/xxd-panel-082 photo.jpg --mode design-only --size 3:4 --text prompt --locale en-GB

# Use exact copy without rewriting, translating, or adding a title
/xxd-panel-082 photo.jpg --mode design-only --size 3:4 --copy "Let the light stay" --locale en-GB

# Two modes × three sizes produce six independently composed assets
/xxd-panel-082 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale en-GB

# Apply Panel 082 to a whole directory; shared settings resolve once
/xxd-panel-082 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale en-GB

# Repeated parameters accumulate; place the task under a chosen root
/xxd-panel-082 photo.jpg --mode top-bottom --mode left-right --size 3:4 --size 16:9 --text none --out ./deliveries
```
...
```bash
git clone https://github.com/nevertoday/xxd-panel-082.git
mkdir -p ~/.codex/skills
ln -s "$(pwd)/xxd-panel-082" ~/.codex/skills/xxd-panel-082
```

You can also install it directly with `npx skills`:

```bash
npx skills add https://github.com/nevertoday/xxd-panel-082 --skill xxd-panel-082
```
...
Claude Code users may link the same folder under `~/.claude/skills/xxd-panel-082`. Restart the agent session after installation.
...
Full specifications: [Skill workflow](SKILL.md) · [source archive](references/original-prompt/zh-CN.md) · [English runtime adapter](references/xxd-panel-082-prompt.en.md) · [Chinese runtime adapter](references/xxd-panel-082-prompt.zh-CN.md)

<!-- xxd-panel-catalog:start -->
## Complete XXD Panel catalog

The current XXD Panel series runs from 001 through 112, and every Panel retains its own independent original prompt and aesthetic logic. The table below is the historical catalog as of this project’s release, listing 001 through 082; the current project is bold.
