...
`sample-01`–`sample-04` 保留给未来可核验的原始发布样张。以上八张包括四张 16:9 横版左右样张和四张 3:4 竖版上下样张，均由 Panel 082 依据自己的原始提示词独立生成，不借用其他编号作品。样张使用英文智能配字。

<!-- xxd-human-intro:start -->
## 适用场景与解决的问题

普通儿童插画容易变成平面贴纸，写实 3D 又会失去水彩的呼吸；复杂场景若不删减，核心主角与照片隐喻就会被淹没。

**Panel 082** 将原图压缩为大面积不规则水彩色块，再让唯一核心主角以 Naïve + Wonky 的稚拙手绘轮廓和 Isometric／2.5D 轴测结构从色域中“站起来”。层叠平面、前后遮挡和轻微高度差赋予空间感，鲜活水彩从原图提取生命力，大片暖白留白保留空气与童趣。

### 适合这些情况

- **想让主体从水彩色域中跃出：** 用不规则水彩块承载空气与地形，再以 Isometric／2.5D 结构托起唯一主角。
- **需要童趣但不想失去空间逻辑：** 保留 Naïve + Wonky 的手工误差，同时让轴测方向、层叠与遮挡保持清楚。
...
> 用 XXD Panel 082 帮我处理这张图，先推荐最合适的构图和尺寸。
...
**关键词：** 不规则水彩色域 · Naïve + Wonky · Isometric／2.5D · 稚拙手绘轮廓 · 鲜活治愈配色 · 立体小世界

<!-- xxd-panel-benefit:start -->
## 快速判断：XXD Panel 082 适合你吗？

| 你关心的问题 | 这套风格给你的回答 |
|---|---|
| **你会得到什么** | 一幅从柔软水彩平面中跃出立体主角的高级绘本海报 |
| **一眼可辨的特点** | 不规则水彩底层、稚拙轮廓、清晰轴测方向、层叠平面与大面积留白 |
| **它如何尊重你的输入** | 只保留最重要的身份特征与视觉隐喻；颜色从原图最明亮鲜活的部分重新调制 |
...
```text
提取主体、结构、姿态、关系与隐喻 → 以不规则水彩色块建立柔软舞台 → 用 Naïve + Wonky 轮廓概括核心主角 → 以 Isometric／2.5D 轴测、层叠平面与遮挡制造空间 → 从原图提取鲜活颜色并提升明度纯净度 → 让主体局部跃出色块边界 → 用暖白留白和少量圆润文字完成叙事
```

## 成品中最容易识别的风格特点

- 先用自然水痕、颗粒、浓淡、透底和轻微错位的大色块建立基础构图。
- 核心主角保持稚拙、不对称和手工误差，但轴测视角、结构方向与空间关系必须清楚。
- 立体主体的关键色块更实更鲜明，背景色域柔和透明，主体可局部越出边界。
- 配色从照片最明亮、鲜活、有生命力的颜色提取，避免灰脏、荧光、廉价糖果感。
- 文字纤细圆润、少量克制，可沿色块边缘、轴线或留白自然排布。

原始审美约束与拒绝项只存在于[原始提示词](references/original-prompt/zh-CN.md)；Skill 与运行适配器只处理本次交付变量。 [Skill 工作流](SKILL.md) · [英文运行适配器](references/xxd-panel-082-prompt.en.md)
...
直接传入一个图片目录时，这个士兵 Skill 会自动批量处理目录中的图片，并让每张图片都使用 Panel 082 的同一套原始审美。默认递归扫描常见位图，先报告数量，再统一确认一次模式、尺寸、文字和语言；每张图片独立生成，互不借用内容或文案。整批仍然只创建一个扁平任务目录，单张失败会被记录，但不会静默跳过后续图片。
...
/xxd-panel-082 photo.jpg --mode top-bottom --size auto --text prompt --locale zh-CN
...
# 使用准确文案，不改写、不翻译、不补标题
/xxd-panel-082 photo.jpg --mode design-only --size 3:4 --copy "让风经过这里" --locale zh-CN

# 两种模式 × 三种尺寸，共生成六张独立完整构图
/xxd-panel-082 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale zh-CN

# 整个目录统一使用 Panel 082；公共设置只解析一次
/xxd-panel-082 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale zh-CN
...
Claude Code 用户可把同一文件夹链接到 `~/.claude/skills/xxd-panel-082`. 安装后请重启 Agent 会话。
...
完整规格: [Skill 工作流](SKILL.md) · [原始风格档案](references/original-prompt/zh-CN.md) · [英文运行适配器](references/xxd-panel-082-prompt.en.md) · [中文运行适配器](references/xxd-panel-082-prompt.zh-CN.md)

<!-- xxd-panel-catalog:start -->
## XXD Panel 全系列项目

82 个 Panel 各自保留独立的原始提示词与审美逻辑。下面连续列出 001–082 的全部项目地址和核心风格特点；当前项目以粗体标出。
...
| **士兵级** | `xxd-panel-NNN`（当前 001–082） | 每个编号只执行自己独立的原始提示词与审美，把将军派发的单个任务完成为成品。 |
