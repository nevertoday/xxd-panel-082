...
`sample-01`–`sample-04`는 검증 가능한 원본 공개 예시를 위해 남겨 둡니다. 위 여덟 작품은 16:9 가로형 좌우 구성 네 장과 3:4 세로형 상하 구성 네 장입니다. 모두 Panel 082이 자체 원본 프롬프트에 따라 독립적으로 생성했으며 다른 번호의 작품을 재사용하지 않았습니다. 예시 문구는 영어로 자동 생성했습니다.
...
```text
정체성·구조·자세·시각 은유 추출 → 장면을 불규칙 수채 색면으로 압축 → Naïve + Wonky 주인공 하나 설정 → 겹친 평면·절단면·가림으로 Isometric／2.5D 깊이 구성 → 원본의 가장 생생한 색 재조정 → 따뜻한 흰 여백 유지 → 색면과 축을 따라 소량의 편집 글자 배치
```

## 완성작의 특징

- 넓은 불규칙 수채 색면에 물자국, 입자, 투명감, 미세한 어긋남이 보입니다.
- 하나의 주인공이 축측 방향, 겹친 평면, 가림으로 평면 색역에서 일어납니다.
- 서툰 비율과 떨리는 윤곽은 명확한 Isometric 공간 논리로 통제됩니다.
- 색은 원본의 가장 생생한 부분에서 가져오고 배경은 부드럽게, 피사체 핵심색은 조금 더 단단하게 합니다.
- 복잡한 환경을 지우고 양형과 음형, 공기, 하나의 시각 중심으로 화면을 지배합니다.

미적 제약은 [원본 프롬프트](references/original-prompt/zh-CN.md)에만 있습니다. Skill과 실행 어댑터는 납품 변수만 처리합니다. [Skill](SKILL.md) · [영문 실행 어댑터](references/xxd-panel-082-prompt.en.md)
...
82개 Panel은 각각 독립된 원본 프롬프트와 미적 논리를 유지합니다. 아래 표에는 001부터 082까지 빠짐없이 나열하며 현재 프로젝트는 굵게 표시합니다.
...
| **병사급** | `xxd-panel-NNN` (현재 001–082) | 각 번호가 고유한 원본 프롬프트와 미학만 실행해 장군이 배정한 하나의 작업을 완성합니다. |
