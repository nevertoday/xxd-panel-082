...
<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-05.png" alt="XXD Panel 082 Sample 5"></td>
    <td width="50%"><img src="./assets/examples/sample-06.png" alt="XXD Panel 082 Sample 6"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-07.png" alt="XXD Panel 082 Sample 7"></td>
    <td width="50%"><img src="./assets/examples/sample-08.png" alt="XXD Panel 082 Sample 8"></td>
  </tr>
</table>
...
<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-09.png" alt="XXD Panel 082 additional top-bottom sample 1"></td>
    <td width="50%"><img src="./assets/examples/sample-10.png" alt="XXD Panel 082 additional top-bottom sample 2"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-11.png" alt="XXD Panel 082 additional top-bottom sample 3"></td>
    <td width="50%"><img src="./assets/examples/sample-12.png" alt="XXD Panel 082 additional top-bottom sample 4"></td>
  </tr>
</table>
...
위 여덟 작품은 16:9 가로형 좌우 구성 네 장과 3:4 세로형 상하 구성 네 장입니다. 모두 Panel 082이 자체 원본 프롬프트에 따라 독립적으로 생성했으며 다른 번호의 작품을 재사용하지 않았습니다. 예시 문구는 영어로 자동 생성했습니다.

<!-- xxd-human-intro:start -->
## 이 스타일이 해결하는 문제

평면적인 아동 일러스트는 스티커처럼 되기 쉽고 사실적 3D는 수채의 호흡을 잃습니다. 복잡한 장면을 줄이지 않으면 핵심 주인공과 사진의 은유가 묻힙니다.

**Panel 082**는 사진을 넓은 불규칙 수채 색면으로 압축하고 하나의 주인공을 Naïve + Wonky 윤곽과 명확한 Isometric／2.5D 구조로 일으킵니다. 겹친 평면, 가림, 높이 차이가 공간을 만들고 원본의 생생한 색과 따뜻한 흰 여백이 공기와 동심을 남깁니다.

### 이런 경우에 적합합니다

- 피사체를 불규칙 수채 색역에서 Isometric／2.5D 깊이로 튀어나오게 하고 싶을 때
- Naïve + Wonky의 동심을 유지하면서 방향, 겹침, 공간 논리를 잃고 싶지 않을 때
- 복잡한 사진을 정체성, 은유, 가장 생명력 있는 원본 색만으로 줄이고 싶을 때
- 상하·좌우·디자인 전용·여러 비율·벽지·폴더 일괄 처리

### 빠른 시작

> 이 이미지를 XXD Panel 082로 처리하고 가장 알맞은 구도와 크기를 먼저 추천해 주세요.
...
| 확인할 점 | 이 스타일의 답 |
|---|---|
| **결과물** | 부드러운 수채 평면에서 입체 주인공이 일어나는 고급 그림책 포스터 |
| **대표 특징** | 불규칙 수채, 서툰 윤곽, 명확한 축측 방향, 겹친 평면, 넓은 여백 |
| **입력 존중 방식** | 가장 중요한 정체성과 시각 은유를 남기고 원본의 가장 밝고 생생한 색을 재조정함 |
...
Claude Code 사용자는 같은 폴더를 다음 위치에 연결할 수 있습니다: `~/.claude/skills/xxd-panel-082`. 설치 후 에이전트 세션을 다시 시작하세요.
...
전체 사양: [Skill 워크플로](SKILL.md) · [원본 스타일 자료](references/original-prompt/zh-CN.md) · [영문 런타임 어댑터](references/xxd-panel-082-prompt.en.md) · [중문 런타임 어댑터](references/xxd-panel-082-prompt.zh-CN.md)
...
**특징:** 불규칙 수채 색역 · Naïve + Wonky · Isometric／2.5D · 소박한 손그림 윤곽 · 생생한 치유색 · 작은 입체 세계
...
이미지 폴더를 직접 전달하면 이 병사 Skill이 같은 Panel 082의 원본 미학으로 일괄 처리합니다. 일반 래스터 형식을 기본적으로 재귀 탐색하고 수량을 먼저 알린 뒤 모드·크기·텍스트·언어를 한 번만 확인합니다. 각 이미지는 내용이나 문구가 섞이지 않도록 독립 생성하며 전체 배치는 하나의 평면 작업 디렉터리만 사용합니다. 개별 실패는 기록하고 이후 이미지를 조용히 누락하지 않습니다.
...
/xxd-panel-082 photo.jpg --mode top-bottom --size auto --text prompt --locale ko-KR

# 좌우 비교: 3:2 가로형, 글자 없음
/xxd-panel-082 photo.jpg --mode left-right --size 3:2 --text none

# 디자인 전용: 9:16 휴대폰 세로형, 한국어 문구
/xxd-panel-082 photo.jpg --mode design-only --size 9:16 --text prompt --locale ko-KR
...
/xxd-panel-082 photo.jpg --mode design-only --size 3:4 --text prompt --locale ko-KR

# 정확한 문구를 재작성·번역·제목 추가 없이 사용
/xxd-panel-082 photo.jpg --mode design-only --size 3:4 --copy "빛을 이곳에 남겨 두기" --locale ko-KR

# 2개 모드 × 3개 크기로 독립적인 완성 구도 6개 생성
/xxd-panel-082 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale ko-KR

# 폴더 전체에 Panel 082 적용, 공통 설정은 한 번만 확정
/xxd-panel-082 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale ko-KR
...
현재 XXD Panel 시리즈는 001–112까지 업데이트되었으며, 각 Panel은 독립된 원본 프롬프트와 미적 논리를 유지합니다. 아래 표는 이 프로젝트 공개 시점의 과거 목록으로 001–082을 연속해서 나열하며, 현재 프로젝트는 굵게 표시합니다.
