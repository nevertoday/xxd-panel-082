...
`sample-01`–`sample-04` は、検証可能なオリジナル公開作例のために確保しています。上の8点は、16:9の横長・左右構成4点と、3:4の縦長・上下構成4点です。すべて Panel 082 が自身の原文プロンプトに基づいて個別に生成したもので、別番号の作品は流用していません。作例のコピーは英語で自動生成しています。
...
```text
同一性・構造・姿勢・視覚比喩を抽出 → 場面を不規則な水彩色面へ圧縮 → Naïve + Wonky の主役を一つ定める → 重なる平面・切面・遮蔽で Isometric／2.5D の奥行きを作る → 元写真の最鮮色を再調整 → 暖白の余白を残す → 色面と軸に沿って少量の編集文字を置く
```

## 完成作の特徴

- 大きな不規則水彩色面に水跡、粒子、透明感、わずかなずれが見える。
- 一つの主役が軸測方向、重なる平面、遮蔽によって平面色域から立ち上がる。
- 素朴な比率と震える輪郭は、明確な Isometric の空間論理で制御される。
- 色は元写真の最も鮮やかな部分から取り、背景を柔らかく、主体の要色を少し実在的にする。
- 複雑な環境を消し、正負形、空気、一つの視覚中心で紙面を統御する。

美的制約は[原文プロンプト](references/original-prompt/zh-CN.md)だけにあります。Skill と実行アダプターは納品変数のみを扱います。 [Skill](SKILL.md) · [英語実行アダプター](references/xxd-panel-082-prompt.en.md)
...
82 個の Panel は、それぞれ独立した原文プロンプトと美的ロジックを保持しています。表には 001 から 082 までを欠番なく掲載し、現在のプロジェクトを太字で示します。
...
| **兵士級** | `xxd-panel-NNN`（現在 001–082） | 各番号が固有の原文プロンプトと美学だけを実行し、将軍から渡された一つの仕事を完成させます。 |
