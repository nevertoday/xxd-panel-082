...
上の8点は、16:9の横長・左右構成4点と、3:4の縦長・上下構成4点です。すべて Panel 082 が自身の原文プロンプトに基づいて個別に生成したもので、別番号の作品は流用していません。作例のコピーは英語で自動生成しています。

<!-- xxd-human-intro:start -->
## このスタイルが解決すること

平面的な児童イラストはステッカー化しやすく、写実的3Dは水彩の呼吸を失います。複雑な場面を削らなければ、中心人物と写真の比喩が埋もれます。

**Panel 082** は写真を大きな不規則水彩色面へ圧縮し、唯一の主役を Naïve + Wonky の輪郭と明確な Isometric／2.5D 構造で立ち上げます。重なる平面、遮蔽、高低差が空間を作り、元写真由来の鮮色と暖白の余白が空気と童心を残します。

### 向いている用途

- 主体を不規則な水彩色域から Isometric／2.5D の奥行きで飛び出させたい場合
- Naïve + Wonky の童心を保ちながら、方向、重なり、空間論理を失いたくない場合
- 複雑な写真を同一性、比喩、最も生命力のある元色だけへ減らしたい場合
- 上下・左右・デザインのみ・複数比率・壁紙・フォルダ一括処理

### すぐ試す

> この画像を XXD Panel 082 で処理し、最適な構図とサイズを先に提案してください。
...
| 確認したいこと | このスタイルの答え |
|---|---|
| **得られるもの** | 柔らかな水彩平面から立体的な主役が立ち上がる上質な絵本ポスター |
| **識別しやすい特徴** | 不規則水彩、素朴な輪郭、明確な軸測方向、重なる平面、大きな余白 |
| **入力の尊重** | 最重要の同一性と視覚比喩を残し、元写真の最も明るく鮮やかな色を再調整する |
...
Claude Code では同じフォルダを次へリンクできます： `~/.claude/skills/xxd-panel-082`. インストール後に Agent セッションを再起動してください。
...
完全仕様: [Skill ワークフロー](SKILL.md) · [原始スタイル資料](references/original-prompt/zh-CN.md) · [英語ランタイムアダプター](references/xxd-panel-082-prompt.en.md) · [中国語ランタイムアダプター](references/xxd-panel-082-prompt.zh-CN.md)
...
**特徴：** 不規則な水彩色域 · Naïve + Wonky · Isometric／2.5D · 素朴な輪郭 · 鮮やかな癒し色 · 小さな立体世界
...
画像フォルダーを直接渡すと、この兵士 Skill が同じ Panel 082 の原文美学で一括処理します。一般的なラスター形式を既定で再帰検索し、件数を報告してから、モード・サイズ・文字・言語を一度だけ確認します。各画像は内容や文言を混ぜず独立生成し、バッチ全体は一つのフラットなタスクフォルダーへ保存します。個別の失敗は記録し、後続画像を黙って省略しません。
...
/xxd-panel-082 photo.jpg --mode top-bottom --size auto --text prompt --locale ja-JP
...
/xxd-panel-082 photo.jpg --mode design-only --size 3:4 --text prompt --locale ja-JP

# 正確な文言を、書き換え・翻訳・タイトル追加なしで使用
/xxd-panel-082 photo.jpg --mode design-only --size 3:4 --copy "光をここに残す" --locale ja-JP

# 2モード × 3サイズで独立した完成構図を6枚生成
/xxd-panel-082 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale ja-JP

# フォルダー全体へ Panel 082 を適用し、共通設定は一度だけ解決
/xxd-panel-082 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale ja-JP
...
XXD Panel の現行シリーズは 001〜112 まで更新され、各 Panel は独立した原文プロンプトと美的ロジックを保持しています。以下は本プロジェクト公開時点の履歴カタログで、001〜082 を連続して掲載し、現在のプロジェクトを太字で示します。
